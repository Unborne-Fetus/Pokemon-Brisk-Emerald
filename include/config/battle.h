...
#define B_FLAG_DYNAMAX_BATTLE       0x23  // If this flag is set, the ability to Dynamax in battle is enabled for all Trainers.
#define B_FLAG_TERA_ORB_CHARGED     0x24  // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_OVERWORLD_WEATHER_OVERRIDE    GEN_7 // In Gen9+, overworld weather cannot be overridden.
...
#define B_CATCHING_CHARM_BOOST          150        // % increase in critical capture odds if the player has the Catching Charm.
...
#define B_LOW_LEVEL_CATCH_BONUS         GEN_5 // In Gen9+, a bonus is added to the catch rate when catching a Pokémon lower than level 13. In Gen8, the bonus is applied to Pokémon lower than level 20.
#define B_MISSING_BADGE_CATCH_MALUS     GEN_5 // In Gen9+, a penalty is added to the catch rate when trying to catch a Pokémon 5 levels above the current obedience level, based on the number of Badges obtained.
...
#define B_TRAINER_MON_RANDOM_ABILITY    2         // If this is set to 0, trainers mons with no set ability only use the first ability of a mon. If set to 1 it will roll a random non-hidden ability. If set to 2, it will roll any ability (including hidden ability) with equal chance
#define B_OBEDIENCE_MECHANICS           GEN_6 // In PLA onwards (GEN_8 here), obedience restrictions also apply to non-outsider Pokémon, albeit based on their met level rather than their current level.
...
#define B_TRY_CATCH_TRAINER_BALL        GEN_3 // In Gen4+, trying to catch a Trainer's Pokémon no longers consume the Poké Ball.
...
#define NUM_BEEPS_OFF                   1                    // Does not play at all
#define B_NUM_LOW_HEALTH_BEEPS          NUM_BEEPS_OFF // This controls the number of times the "low HP" beep will loop. Setting this value to NUM_BEEPS_OFF will disable the beep, while NUM_BEEPS_GEN_3 will loop infinitely. You can set this to any number you want, the defines listed are just for ease of use.
